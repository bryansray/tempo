...
module ApplicationHelper
  ...
  def generic_update_textbox( object, field, options = {} )
    ...
    generic_update_field( object, field, nil, nil, options )
  end
  
  #Creates link and select elements for a given model and field that can be toggled and updated via Ajax.
  def generic_update_select( object, field, options_list = nil, options = {} )
    ...
    options_list.insert(0, options_for_select( [[null_text, -1]], field_obj.nil? ? -1 : 0) )
    
    generic_update_field( object, field, field_obj, options_list, options )
  end
  
  def generic_update_field( object, field, field_obj, options_list = nil, options = {} )
    #get instance of referenced field for use later
    field_obj = object.send(field) if field_obj.nil?
    ...
    if options_list.nil? #do INPUT ids
      #this is the html id attribute values to be used for the link and form field
    ...
    if options_list.nil? #do INPUT fields
    ...
    else #do SELECT fields
      out << select_tag( text_id, options_list, options )
